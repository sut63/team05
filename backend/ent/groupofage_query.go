...
// QueryGropages chains the current query on the gropages edge.
func (goaq *GroupOfAgeQuery) QueryGropages() *ProductQuery {
...
//  WithGropages tells the query-builder to eager-loads the nodes that are connected to
// the "gropages" edge. The optional arguments used to configure the query builder of the edge.
func (goaq *GroupOfAgeQuery) WithGropages(opts ...func(*ProductQuery)) *GroupOfAgeQuery {
	query := &ProductQuery{config: goaq.config}
	for _, opt := range opts {
		opt(query)
	}
	goaq.withGropages = query
	return goaq
}
...
func (goaq *GroupOfAgeQuery) sqlAll(ctx context.Context) ([]*GroupOfAge, error) {
	...
	if query := goaq.withGropages; query != nil {
		...
		query.Where(predicate.Product(func(s *sql.Selector) {
			s.Where(sql.InValues(groupofage.GropagesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Gropages = append(node.Edges.Gropages, n)
		}
	}
