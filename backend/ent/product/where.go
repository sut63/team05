...
// HasProductPayment applies the HasEdge predicate on the "product_payment" edge.
func HasProductPayment() predicate.Product {
	return predicate.Product(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(ProductPaymentTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, ProductPaymentTable, ProductPaymentColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasProductPaymentWith applies the HasEdge predicate on the "product_payment" edge with a given conditions (other predicates).
func HasProductPaymentWith(preds ...predicate.Payment) predicate.Product {
	return predicate.Product(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(ProductPaymentInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, ProductPaymentTable, ProductPaymentColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
