...
func (pq *PositionQuery) QueryMembers() *MemberQuery {
...
func (pq *PositionQuery) QueryOfficers() *OfficerQuery {
...
func (pq *PositionQuery) WithMembers(opts ...func(*MemberQuery)) *PositionQuery {
	query := &MemberQuery{config: pq.config}
...
func (pq *PositionQuery) WithOfficers(opts ...func(*OfficerQuery)) *PositionQuery {
	query := &OfficerQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withOfficers = query
	return pq
}
...
func (pq *PositionQuery) sqlAll(ctx context.Context) ([]*Position, error) {
	...
	if query := pq.withMembers; query != nil {
		...
		query.Where(predicate.Member(func(s *sql.Selector) {
			s.Where(sql.InValues(position.MembersColumn, fks...))
		}))
	...
	if query := pq.withOfficers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Position)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.Officer(func(s *sql.Selector) {
			s.Where(sql.InValues(position.OfficersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
