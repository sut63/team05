...
func (iq *InsuranceQuery) QueryMember() *MemberQuery {
...
func (iq *InsuranceQuery) QueryHospital() *HospitalQuery {
	query := &HospitalQuery{config: iq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *InsuranceQuery) QueryOfficer() *OfficerQuery {
...
func (iq *InsuranceQuery) QueryProduct() *ProductQuery {
...
func (iq *InsuranceQuery) WithMember(opts ...func(*MemberQuery)) *InsuranceQuery {
	query := &MemberQuery{config: iq.config}
...
func (iq *InsuranceQuery) WithHospital(opts ...func(*HospitalQuery)) *InsuranceQuery {
	query := &HospitalQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withHospital = query
	return iq
}
...
func (iq *InsuranceQuery) WithOfficer(opts ...func(*OfficerQuery)) *InsuranceQuery {
	query := &OfficerQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *InsuranceQuery) WithProduct(opts ...func(*ProductQuery)) *InsuranceQuery {
	query := &ProductQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(insurance.FieldInsuranceIdentification).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Insurance.Query().
//		Select(insurance.FieldInsuranceIdentification).
//		Scan(ctx, &v)
//
...
func (iq *InsuranceQuery) sqlAll(ctx context.Context) ([]*Insurance, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, insurance.ForeignKeys...)
	}
	...
	if query := iq.withHospital; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Insurance)
		for i := range nodes {
