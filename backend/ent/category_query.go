...
type CategoryQuery struct {
	...
	// eager-loading edges.
	withCategoryInquiry *InquiryQuery
...
// QueryCategoryInquiry chains the current query on the category_inquiry edge.
func (cq *CategoryQuery) QueryCategoryInquiry() *InquiryQuery {
	query := &InquiryQuery{config: cq.config}
...
//  WithCategoryInquiry tells the query-builder to eager-loads the nodes that are connected to
// the "category_inquiry" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CategoryQuery) WithCategoryInquiry(opts ...func(*InquiryQuery)) *CategoryQuery {
	query := &InquiryQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCategoryInquiry = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withCategoryInquiry; query != nil {
		...
		query.Where(predicate.Inquiry(func(s *sql.Selector) {
			s.Where(sql.InValues(category.CategoryInquiryColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.CategoryInquiry = append(node.Edges.CategoryInquiry, n)
		}
	}
