...
// QueryHospitalInsurance chains the current query on the hospital_insurance edge.
func (hq *HospitalQuery) QueryHospitalInsurance() *InsuranceQuery {
	query := &InsuranceQuery{config: hq.config}
...
//  WithHospitalInsurance tells the query-builder to eager-loads the nodes that are connected to
// the "hospital_insurance" edge. The optional arguments used to configure the query builder of the edge.
func (hq *HospitalQuery) WithHospitalInsurance(opts ...func(*InsuranceQuery)) *HospitalQuery {
	query := &InsuranceQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withHospitalInsurance = query
	return hq
}
...
func (hq *HospitalQuery) sqlAll(ctx context.Context) ([]*Hospital, error) {
	...
	if query := hq.withHospitalInsurance; query != nil {
		...
		query.Where(predicate.Insurance(func(s *sql.Selector) {
			s.Where(sql.InValues(hospital.HospitalInsuranceColumn, fks...))
		}))
